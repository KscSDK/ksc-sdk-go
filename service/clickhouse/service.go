...
// Clickhouse provides the API operation methods for making requests to
// clickhouse. See this package's package overview docs
// for details on the service.
//
// Clickhouse methods are safe to use concurrently. It is not safe to
...
const (
	ServiceName = "clickhouse" // Name of service.
	EndpointsID = ServiceName  // ID to lookup a service endpoint with.
	ServiceID   = "clickhouse" // ServiceID is a unique identifer of a specific service.
)

// New creates a new instance of the Clickhouse client with a session.
// If additional configuration is needed for the client instance use the optional
// ksc.Config parameter to add your extra config.
//
// Example:
//     // Create a Clickhouse client from just a session.
//     svc := clickhouse.New(mySession)
//
//     // Create a Clickhouse client with additional configuration
//     svc := clickhouse.New(mySession, aws.NewConfig().WithRegion("us-west-2"))
func New(p client.ConfigProvider, cfgs ...*aws.Config) *Clickhouse {
...
func ExtraNew(info *utils.UrlInfo, p client.ConfigProvider, cfgs ...*aws.Config) *Clickhouse {
	c := p.ClientConfig(EndpointsID, cfgs...)
...
func SdkNew(p client.ConfigProvider, cfgs *ksc.Config, info ...*utils.UrlInfo) *Clickhouse {
	_info := utils.UrlInfo{}
...
func newClient(cfg aws.Config, handlers request.Handlers, endpoint, signingRegion, signingName string) *Clickhouse {
	svc := &Clickhouse{
		Client: client.New(
			cfg,
			metadata.ClientInfo{
...
// newRequest creates a new request for a Clickhouse operation and runs any
// custom request initialization.
func (c *Clickhouse) newRequest(op *request.Operation, params, data interface{}) *request.Request {
