...
// Kpfs provides the API operation methods for making requests to
// kpfs. See this package's package overview docs
// for details on the service.
//
// Kpfs methods are safe to use concurrently. It is not safe to
...
const (
	ServiceName = "kpfs"      // Name of service.
	EndpointsID = ServiceName // ID to lookup a service endpoint with.
	ServiceID   = "kpfs"      // ServiceID is a unique identifer of a specific service.
)

// New creates a new instance of the Kpfs client with a session.
// If additional configuration is needed for the client instance use the optional
// ksc.Config parameter to add your extra config.
//
// Example:
//
//	// Create a Kpfs client from just a session.
//	svc := Kpfs.New(mySession)
//
//	// Create a Kpfs client with additional configuration
//	svc := Kpfs.New(mySession, aws.NewConfig().WithRegion("us-west-2"))
func New(p client.ConfigProvider, cfgs ...*aws.Config) *Kpfs {
...
func ExtraNew(info *utils.UrlInfo, p client.ConfigProvider, cfgs ...*aws.Config) *Kpfs {
	c := p.ClientConfig(EndpointsID, cfgs...)
...
func SdkNew(p client.ConfigProvider, cfgs *ksc.Config, info ...*utils.UrlInfo) *Kpfs {
	_info := utils.UrlInfo{}
...
func newClient(cfg aws.Config, handlers request.Handlers, endpoint, signingRegion, signingName string) *Kpfs {
	svc := &Kpfs{
		Client: client.New(
			cfg,
			metadata.ClientInfo{
...
// newRequest creates a new request for a Kpfs operation and runs any
// custom request initialization.
func (c *Kpfs) newRequest(op *request.Operation, params, data interface{}) *request.Request {
